...
int main()
{
	...
	for (int richieste = 0; richieste < 2; richieste++)
	{
		...
		/* riceve un messaggio dal client (qualsiasi tipo) */
		// riceve qualsiasi richiesta, da qualsiasi client
		ssize_t msgrcv_ret = msgrcv(rcv_queue, &req, sizeof(msg_init_request) - sizeof(long), 0, 0);
		if (msgrcv_ret < 0)
		{
			perror("msgrcv lato server");
			printf("msgrcv returned %zd, errno=%d\n", msgrcv_ret, errno);
			exit(1);
		}
		...
		printf("[SERVER %d] Ricezione richiesta (num. valori: %d, pid: %ld)\n", getpid(), num_valori, req.pid);
		...
		int ret = msgsnd(send_queue, &res, sizeof(msg_init_response) - sizeof(long), 0);
		if (ret < 0)
		{
...
void server(prodcons *p1, prodcons *p2, int val, int id_sem1, int id_sem2)
{
	...
	for (int i = 0; i < num_valori; i++)
	{
		...
		printf("[SERVER %d] Valore ricevuto: %d (%d)\n", getpid(), ricevuto, ricevuto * 2);
