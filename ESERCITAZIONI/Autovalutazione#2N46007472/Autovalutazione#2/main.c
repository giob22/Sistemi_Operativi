...
int main(){
        ...
        int key_queue_filter_checksum = ftok("./main.c", 'c'); /* TODO: definire chiave coda filtro-checksum */
        int key_queue_checksum_visual = ftok("./main.c", 'v'); /* TODO: definire chiave coda checksum-visualizzatore */
        ...
        if (ds_queue_checksum_visual < 0 || ds_queue_filter_checksum < 0 || ds_queue_gen_filter < 0)
        {
            LOG("f: %d, c: %d, v: %d", ds_queue_gen_filter, ds_queue_filter_checksum, ds_queue_checksum_visual);
            exit(-1);
        }
        ...
        key_t shm_key = ftok("./generatore.c", 'a'); /* TODO: definire chiave shared memory per prod/cons generatori*/
        ...
        shmctl(ds_shm, IPC_RMID, 0); // marco come eliminabile la shm
        // elimino istantaneamente le messagge queue
