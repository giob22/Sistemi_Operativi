...
int main(){
    ...
    }else
    {
        ...
        printf("[ELABORATORE: %d] la shm non è stata creata dal master\n", getpid());
        exit(1);
    }
    ...
    // legge il contenuto della stringa, converte in miuscolo ogni carattere e stampa a video il risultato
    int indice = 0;
    while (indice < NUM_RIPETIZIONI)
    {
        ...
        {
            if (buf->testo[i] <= 'z' && buf->testo[i] >= 'a')
            {
                t[i] = buf->testo[i] - (char)32;
            }
            
        }
        t[buf->dim] = '\0';
        printf("[ELABORATORE: %d] risultato dell'elaborazione: %s\n", getpid(), t);
